...
/** Permanently delete a diary entry row. */
 ...
 */
export function upsertHitMetadata(db, clipId, { timestamps, confidences, loudnesses, paddingS }) {
  const now = new Date().toISOString();
  db.prepare(`
    INSERT INTO hit_metadata (clip_id, timestamps, confidences, loudnesses, padding_s, created_at)
    VALUES (?, ?, ?, ?, ?, ?)
...
export function getHitMetadata(db, clipId) {
  const row = db.prepare(
    "SELECT clip_id AS clipId, timestamps, confidences, loudnesses, padding_s AS paddingS, created_at AS createdAt FROM hit_metadata WHERE clip_id = ?"
  ).get(clipId);
  if (!row) return null;
  return {
    clipId: row.clipId,
